"""program for single player racecar game - v9iii
change player score system
...
try:
    ...
    ROAD_SURFACE = pygame.image.load("images/5_lane_road.png").convert()
...
class Car:
    ...
    def car_turn(self, current_direction):
        ...
        if self.rect.bottom > SCREEN_HEIGHT: self.rect.top = SCREEN_HEIGHT
    ...
    while play:
        ...
        if not game_paused and not game_over:
            ...
            if len(npc_cars) == 0 or npc_cars[-1].y < (SCREEN_HEIGHT - 350):
                x_options = [(SCREEN_WIDTH/10-25), (SCREEN_WIDTH/10*3-25),
                             (SCREEN_WIDTH/10*5-25),
                             (SCREEN_WIDTH/10*7-25),
                             (SCREEN_WIDTH/10*9-25)]
                x_choice1 = random.choice(x_options)
                # cars shouldn't spawn in same lane
                x_options.remove(x_choice1)
                x_choice2 = random.choice(x_options)

                # randomize npc car image
                car_options = [car2_surface, car3_surface, car4_surface,
                               car5_surface, car6_surface]

                car_choice1 = random.choice(car_options)
                # car shouldn't be same car as before
                car_options.remove(car_choice1)
                og_image_npc1 = car_choice1

                car_choice2 = random.choice(car_options)
                og_image_npc2 = car_choice2


                # resize npc car image
                npc_surface1 = pygame.transform.scale(og_image_npc1,
                                                     (70, 140))
                npc_surface2 = pygame.transform.scale(og_image_npc2,
                                                     (70, 140))

                # pass SCREEN_HEIGHT + 10 as starting y
                npc_cars.append(Car(npc_surface1, x_choice1,
                                    SCREEN_HEIGHT + 10,
                                    random.randint(2, 6)))
                npc_cars.append(Car(npc_surface2, x_choice2,
                                    SCREEN_HEIGHT + 10,
                                    random.randint(2, 6)))
            ...
            for car in npc_cars[:]:
                ...
                if car.y < -140:
                    npc_cars.remove(car)
                    score += 1  # increase score if user has successfully
                    # passed car
